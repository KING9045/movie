...
// Uses axios — no shell/curl exec (prevents injection risk)
...
async function syncVidsrc(limitPages = 1, isFullSync = false) {
    // Note: initDB() is NOT called here — caller (server.js) handles it at startup
    let totalSynced = 0;
            ...
            try {
                ...
                for (let i = 0; i < items.length; i += batchSize) {
                    ...
                    await Promise.all(batch.map(async (item) => {
                        // Check if we already have poster/metadata for this (tmdb_id, media_type)
                        const existing = await query(
                            "SELECT poster_path FROM movies WHERE tmdb_id = ? AND media_type = ?",
                            [item.tmdb_id, ep.type]
                        );
                        ...
                        if (!poster_path && process.env.TMDB_API_KEY) {
                            try {
                                const tmdbData = await robustFetch(
                                    `${TMDB_API_BASE}${ep.tmdb}${item.tmdb_id}`,
                                ...
                                // Skip TMDB enrichment errors — continue with basic data
                            }
                        }

                        // Use composite PK (tmdb_id, media_type) — prevents movie/TV collision
                        await query(`
                            INSERT INTO movies (tmdb_id, media_type, imdb_id, title, quality, embed_url, poster_path, overview, release_date, vote_average, genre_ids)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                        ...
                        `, [
                            item.tmdb_id, ep.type, item.imdb_id, item.title, item.quality, item.embed_url,
...
if (require.main === module) {
    ...
    initDB().then(() => syncVidsrc(pages, process.argv[2] === 'full')).catch(err => console.error(err));
}
